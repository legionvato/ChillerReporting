...
# Photo layout in PDF (2-column grid)
PDF_PHOTO_COLS = 2
...
class PDFWriter:
    ...
    def _photo_grid(self, photos: list[bytes], item_label: str):
        """
        Fixes the "giant photo takes whole page" issue.
        Renders photos in a 2-column grid with captions, respecting page breaks.
        """
        if not photos:
            return
        ...
        # Prepare scaled sizes ahead so we can decide row height & page breaks
        scaled = []
        ...
        while i < len(scaled):
            ...
            needed = row_h + 10 * mm  # border+caption spacing
...
def build_docx_bytes(report: dict) -> bytes:
    ...
    for sec in report["sections"]:
        ...
        for item in sec["items"]:
            ...
            if photos:
                # keep docx simple: one photo per paragraph, smaller width than before
                for b in photos:
                    img = Image.open(io.BytesIO(b)).convert("RGB")
                    img_buf = io.BytesIO()
                    img.save(img_buf, format="JPEG", quality=85)
                    ...
                    pic_par.add_run().add_picture(img_buf, width=Inches(4.8))
        doc.add_paragraph()
