...
class PDFWriter:
    ...
    def _badge(self, status: str, x_right: float, y_baseline: float):
        ...
        c = self.c
        if status == "OK":
            fill = colors.HexColor("#EAF4EA")
            stroke = colors.HexColor("#7A9A7A")
            textc = colors.HexColor("#2F5F2F")
        elif status == "Not OK":
            fill = colors.HexColor("#F7EAEA")
            stroke = colors.HexColor("#A05A5A")
            textc = colors.HexColor("#7A1F1F")
        elif status == "N/A":
            fill = colors.HexColor("#F0F0F0")
            stroke = colors.HexColor("#9A9A9A")
            textc = colors.HexColor("#5A5A5A")
        else:
            fill = colors.white
            stroke = colors.HexColor("#C0C0C0")
            textc = colors.HexColor("#555555")
        c.saveState()
    ...
    def _photo_block(self, photos: list[bytes], keep_item_label: str | None = None):
        ...
        for b in photos:
            ...
            if self.y - needed < self.content_bottom:
                ...
                if keep_item_label:
                    self._text_wrapped(f"(continued) {keep_item_label}", size=9, bold=True, color=colors.grey, leading=11)
            x = self.x0
            y = self.y - th
            self.c.saveState()
        ...
        with st.expander(sec["title"], expanded=True):
            for item in sec["items"]:
                ...
                with colB:
                    notes_label = "Notes" 
                    if status == "Not OK":
                        notes_label = "Notes (required for Not OK)"
                    notes = st.text_input(
                        notes_label,
